...
export const BASE_URL = (process.env.HDFC_BASE || 'https://www.hdfilmcehennemi.nl').replace(/\/$/, '');
...
function isBlocked(text, status) {
  ...
  return /Just a moment|Bir dakika lütfen|cf-browser-verification|Yasal Nedenlerle|Please contact the site owner/i.test(head);
}
...
function cleanTranslate(text) {
  return text
    .replace(/https?:\/\/www-hdfilmcehennemi-[a-z]+\.translate\.goog/g, BASE_URL)
    .replace(/[?&]_x_tr_[a-z]+=[^&"'\s]*/g, '');
}

async function tryFetch(url, init, timeoutMs = 20000) {
 ...
 * @param {{api?:boolean, referer?:string, method?:string, body?:any, headers?:object}} opts
 */
export async function smartFetch(pathOrUrl, opts = {}) {
  const { api = true, referer, method = 'GET', body, headers: extra = {} } = opts;
  ...
  // 2) ZenRows / ScraperAPI
  if (process.env.ZENROWS_API_KEY) {
    const zr = `https://api.zenrows.com/v1/?apikey=${process.env.ZENROWS_API_KEY}&url=${encodeURIComponent(url)}&custom_headers=true`;
    const r = await tryFetch(zr, { headers: { 'X-Requested-With': 'fetch', Referer: `${activeBase}/` } }, 60000);
    if (r) return r.text;
  }
  if (process.env.SCRAPERAPI_KEY) {
    const sa = `https://api.scraperapi.com/?api_key=${process.env.SCRAPERAPI_KEY}&keep_headers=true&url=${encodeURIComponent(url)}`;
    const r = await tryFetch(sa, { headers: baseHeaders }, 60000);
    if (r) return r.text;
  }
  ...
  // 3) Google Translate köprüsü
  {
    ...
    if (r) return cleanTranslate(r.text);
  }

  // 4) Public proxy'ler (header taşımaz; HTML sayfalar için işe yarar)
  for (const p of [
...
export function parseCards(html) {
  ...
  anchors.each((_, el) => {
    ...
    if (!href) return;
    href = fixUrl(href);
    if (!/hdfilmcehennemi/.test(href)) return;
...
export async function fetchList(path, page = 1) {
  ...
  if (!data || !data.html || /Sayfa Bulunamadı/.test(data.html)) return { movies: [], hasMore: false };
...
async function getIframeForVideo(videoId, pageUrl) {
  const raw = await smartFetch(`/video/${videoId}/`, { api: true, referer: pageUrl, headers: { 'Content-Type': 'application/json' } });
  if (!raw) return null;
  let html = raw;
  try {
...
async function resolveEmbed(iframeUrl, pageUrl, sourceName) {
  ...
  for (const u of candidates) {
    ...
    const html = await smartFetch(u, { api: false, referer: `${base}/` });
    if (!html) continue;
    const $ = cheerio.load(html);
    let script = '';
    $('script').each((_, s) => {
    ...
    if (!m3u8) {
      // Close: thumbnail dosya adından CDN master.txt türetme (son çare)
...
const CDN_HOSTS = ['srv12.cdnimages96.shop', 'srv12.cdnimages1128.shop', 'srv12.cdnimages1132.shop',
...
async function fixCdnPlaceholder(res) {
  ...
  for (const h of CDN_HOSTS) {
    ...
    if (r && r.text.includes('#EXTM3U')) return { ...res, m3u8: u, referer: EMBED_BASE + '/', origin: EMBED_BASE };
  }
  return null;
}
 ...
 * @returns {{title:string, poster:string, sources:Array<{name,lang,m3u8,referer,origin,subtitles}>}}
 */
export async function resolveMovie(pageUrl, { all = true } = {}) {
  const html = await smartFetch(pageUrl, { api: false });
  if (!html) throw new Error('Film sayfası alınamadı (site erişimi engellenmiş olabilir)');
  const detail = parseDetail(html, pageUrl);
  if (!detail.sources.length) {
    // sayfada gömülü iframe varsa direkt dene
    const $ = cheerio.load(html);
    const ifr = $('iframe').attr('data-src') || $('iframe').attr('src');
    if (ifr) detail.sources.push({ videoId: null, name: 'Varsayılan', lang: '', iframe: fixUrl(ifr) });
  }
  if (!detail.sources.length) throw new Error('Video kaynağı bulunamadı');

  // Rapidrame önce (proxy dostu), sonra Close, sonra diğerleri
  const order = (s) => (/rapidrame/i.test(s.name) ? 0 : /close/i.test(s.name) ? 1 : 2);
  const sorted = [...detail.sources].sort((a, b) => order(a) - order(b));

  const results = [];
  const errors = [];
  for (const s of sorted) {
    try {
      let iframeInfo = s.iframe ? { iframe: s.iframe, cls: '' } : await getIframeForVideo(s.videoId, pageUrl);
      if (!iframeInfo) { errors.push(`${s.name}: iframe yok`); continue; }
      let r = await resolveEmbed(iframeInfo.iframe, pageUrl, s.name + ' ' + iframeInfo.cls);
  ...
  if (!results.length) throw new Error('Hiçbir kaynak çözülemedi: ' + errors.join(' | '));
  return { title: detail.title, poster: detail.poster, year: detail.year, imdb: detail.imdb,
    genres: detail.genres, description: detail.description, sources: results, errors };
}

/* ─────────────────────────── proxy yardımcıları ────────────────────── */

export function encodeProxyUrl(target, referer) {
  const p = new URLSearchParams({ url: target });
  if (referer) p.set('ref', referer);
  return `/api/stream?${p.toString()}`;
}

/** m3u8 içindeki tüm URI'leri proxy'den geçecek şekilde yeniden yazar */
export function rewriteM3u8(text, baseUrl, referer) {
  const abs = (u) => { try { return new URL(u, baseUrl).href; } catch { return u; } };
  return text.split('\n').map((line) => {
    const t = line.trim();
    if (!t) return line;
    if (t.startsWith('#')) {
      return t.replace(/URI="([^"]+)"/g, (_, u) => `URI="${encodeProxyUrl(abs(u), referer)}"`);
    }
    return encodeProxyUrl(abs(t), referer);
  }).join('\n');
}
